 ...
 *
 * En SSR (prerender de Next) no hay window: devolvemos false, el mismo valor
 * que en acceso directo a la app. Así el HTML del server coincide con el primer
 * render del cliente en el caso público, y si además estamos embebidos el
 * `useEffect` de montaje ya corre en el browser con el valor real.
 */
 ...
 * devolvemos la función de baja para llamarla desde el cleanup del useEffect.
 *
 ...
 *
 * En SSR (prerender de Next) no hay window al que suscribirse: no-op que
 * devuelve una función de baja vacía, así el caller no necesita ramificar.
 */
export function listenParentTour(
  ...
  const handler = (ev: MessageEvent) => {
    ...
    if (getView() !== data.view) {
      onView(data.view);
      // Un tick para que React renderice la vista nueva antes de buscar.
