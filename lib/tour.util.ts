...
// El CSS es un import de efecto secundario: lo resuelve el bundler en tiempo de
// build (no ejecuta nada del browser), así que puede quedar en el top-level sin
// romper el prerender. El MÓDULO driver.js, en cambio, toca `document` al
// evaluarse, y por eso se importa dinámicamente dentro de runTour.
...
//
// PORTADO A NEXT: la función es `async` porque driver.js se carga con
// `await import(...)` (no puede evaluarse en el server). Los llamadores la
// invocan SIN await, igual que antes — es fire-and-forget y no devuelve nada
// útil. La única diferencia observable es que el tour arranca un microtask
// después del click en "Ayuda", imperceptible en la práctica.
export async function runTour(steps: DriveStep[], seenKey: string, opts?: { force?: boolean }) {
  // Sin DOM (prerender de Next) no hay nada que resaltar: no-op.
  ...
  // Carga diferida: driver.js sólo se baja cuando el usuario abre un tour.
